"""
Train k-NN classifier: load cached features, tune on val, evaluate on test.

Execution in plain English:
  1. Load the precomputed feature matrices (X) and labels (y) for train, val, and test.
  2. Call the k-NN training routine: it scales the data, runs grid search using the
     validation set to pick the best number of neighbors and distance metric, then
     refits the best model on the training set only.
  3. Scale the test set with the same scaler, predict with the trained model (each
     test point is classified by a majority vote of its nearest training neighbors).
  4. Compute metrics (balanced accuracy, macro F1) and save them plus a confusion
     matrix plot to the results folder.

Usage: python scripts/train_knn.py [--data-config configs/data.yaml] [--model-config configs/model_knn.yaml]
"""
import argparse
import json
...
def main():
    ...
    # --- Train k-NN: scaling, grid search on validation set, then refit best model on train only ---
    print("Training k-NN...")
    ...
    # (each test sample is assigned the majority class among its k nearest training points)
