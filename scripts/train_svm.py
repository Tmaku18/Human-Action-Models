"""
Train SVM baseline: load cached features, tune on val, evaluate on test.

Execution in plain English:
  1. Load the precomputed feature matrices (X) and labels (y) for train, val, and test.
  2. Call the SVM training routine: it scales the data, runs grid search using the
     validation set to pick the best C and gamma, then refits the best model on the
     training set only.
  3. Scale the test set with the same scaler, predict with the trained model.
  4. Compute metrics (balanced accuracy, macro F1) and save them plus a confusion
     matrix plot to the results folder.

Usage: python scripts/train_svm.py [--data-config configs/data.yaml] [--model-config configs/model_svm.yaml]
"""
import argparse
import json
...
from src.models.svm_baseline import train_svm, load_svm_config
...
def main():
    ...
    ap.add_argument("--model-config", type=Path, default=ROOT / "configs" / "model_svm.yaml")
    ap.add_argument("--results-dir", type=Path, default=ROOT / "results" / "baseline_svm")
    args = ap.parse_args()

    # --- Load configs and ensure results directory exists ---
    data_cfg = load_data_config(args.data_config)
    model_cfg = load_svm_config(args.model_config)
    ...
    # --- Train the SVM: scaling, grid search on validation set, then refit best model on train only ---
    print("Training SVM...")
    estimator, scaler, best_params = train_svm(X_train, y_train, X_val, y_val, config=model_cfg)
